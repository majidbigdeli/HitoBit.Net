...
namespace WpfClient.ViewModels
{
    public class MainViewModel : ObservableObject
    {
        ...
        private ObservableCollection<HitoBitSymbolViewModel> allPrices;
        public ObservableCollection<HitoBitSymbolViewModel> AllPrices
        {
        ...
        private HitoBitSymbolViewModel selectedSymbol;
        public HitoBitSymbolViewModel SelectedSymbol
        {
        ...
        private HitoBitSocketClient socketClient;
        ...
        private async Task CloseSettings(object o)
        {
            ...
            if (!string.IsNullOrEmpty(apiKey) && !string.IsNullOrEmpty(apiSecret))
                HitoBitRestClient.SetDefaultOptions(options => { options.ApiCredentials = new ApiCredentials(apiKey, apiSecret); });
        ...
        private async Task GetAllSymbols()
        {
            using (var client = new HitoBitRestClient())
            {
                ...
                if (result.Success)
                    AllPrices = new ObservableCollection<HitoBitSymbolViewModel>(result.Data.Select(r => new HitoBitSymbolViewModel(r.Symbol, r.Price)).ToList());
                else
                    messageBoxService.ShowMessage($"Error requesting data: {result.Error.Message}", "error", MessageBoxButton.OK, MessageBoxImage.Error);
            }

            socketClient = new HitoBitSocketClient();
        ...
        private void OnAccountUpdate(DataEvent<HitoBitStreamPositionsUpdate> data)
        {
        ...
        private void OnOrderUpdate(DataEvent<HitoBitStreamOrderUpdate> data)
        {
