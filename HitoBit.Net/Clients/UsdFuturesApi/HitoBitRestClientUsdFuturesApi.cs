...
{
    /// <inheritdoc cref="IHitoBitRestClientUsdFuturesApi" />
    internal partial class HitoBitRestClientUsdFuturesApi : RestApiClient, IHitoBitRestClientUsdFuturesApi, IFuturesClient
    {
        #region fields 
        /// <inheritdoc />
        ...
        /// <inheritdoc />
        public string ExchangeName => "HitoBit";
        #endregion
        ...
        internal HitoBitRestClientUsdFuturesApi(ILogger logger, HttpClient? httpClient, HitoBitRestOptions options)
            : base(logger, httpClient, options.Environment.UsdFuturesRestAddress!, options, options.UsdFuturesOptions)
        {
            Account = new HitoBitRestClientUsdFuturesApiAccount(this);
            ExchangeData = new HitoBitRestClientUsdFuturesApiExchangeData(logger, this);
            Trading = new HitoBitRestClientUsdFuturesApiTrading(logger, this);
        ...
        internal async Task<HitoBitTradeRuleResult> CheckTradeRules(string symbol, decimal? quantity, decimal? quoteQuantity, decimal? price, decimal? stopPrice, FuturesOrderType type, CancellationToken ct)
        {
            ...
            if (ApiOptions.TradeRulesBehaviour == TradeRulesBehaviour.None)
                return HitoBitTradeRuleResult.CreatePassed(outputQuantity, quoteQuantity, outputPrice, outputStopPrice);
            ...
            if (!symbolData.OrderTypes.Contains(type))
                return HitoBitTradeRuleResult.CreateFailed($"Trade rules check failed: {type} order type not allowed for {symbol}");
        ...
        public IHitoBitRestClientUsdFuturesApiShared SharedClient => this;
        ...
        async Task<WebCallResult<OrderId>> IFuturesClient.PlaceOrderAsync(string symbol, CommonOrderSide side, CommonOrderType type, decimal quantity, decimal? price, int? leverage, string? accountId, string? clientOrderId, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.PlaceOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<Order>> IBaseRestClient.GetOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetOrderAsync), nameof(symbol));
        ...
        async Task<WebCallResult<IEnumerable<UserTrade>>> IBaseRestClient.GetOrderTradesAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetOrderTradesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetClosedOrdersAsync), nameof(symbol));
        ...
        async Task<WebCallResult<OrderId>> IBaseRestClient.CancelOrderAsync(string orderId, string? symbol, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.CancelOrderAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetTickerAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetKlinesAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetOrderBookAsync), nameof(symbol));
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol))
                throw new ArgumentException(nameof(symbol) + " required for HitoBit " + nameof(IFuturesClient.GetRecentTradesAsync), nameof(symbol));
        ...
        private static OrderSide GetOrderSide(CommonOrderSide side)
        {
            ...
            throw new ArgumentException("Unsupported order side for HitoBit order: " + side);
        }
        ...
        private static FuturesOrderType GetOrderType(CommonOrderType type)
        {
            ...
            throw new ArgumentException("Unsupported order type for HitoBit order: " + type);
        }
        ...
        private static KlineInterval GetKlineIntervalFromTimespan(TimeSpan timeSpan)
        {
            ...
            throw new ArgumentException("Unsupported timespan for HitoBit Klines, check supported intervals using HitoBit.Net.Enums.KlineInterval");
        }
