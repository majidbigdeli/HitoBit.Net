...
{
    /// <inheritdoc />
    internal class HitoBitUsdFuturesUserDataSubscription : Subscription<HitoBitSocketQueryResponse, HitoBitSocketQueryResponse>
    {
        ...
        private readonly Action<DataEvent<HitoBitFuturesStreamTradeUpdate>>? _tradeHandler;
        ...
        private readonly Action<DataEvent<HitoBitConditionOrderTriggerRejectUpdate>>? _condOrderHandler;
        ...
        public override Type? GetMessageType(IMessageAccessor message)
        {
            ...
            if (string.Equals(identifier, "TRADE_LITE", StringComparison.Ordinal))
                return typeof(HitoBitCombinedStream<HitoBitFuturesStreamTradeUpdate>);
            ...
            if (string.Equals(identifier, "CONDITIONAL_ORDER_TRIGGER_REJECT", StringComparison.Ordinal))
                return typeof(HitoBitCombinedStream<HitoBitConditionOrderTriggerRejectUpdate>);
        ...
        public HitoBitUsdFuturesUserDataSubscription(
            ILogger logger,
            List<string> topics,
            Action<DataEvent<HitoBitFuturesStreamOrderUpdate>>? orderHandler,
            Action<DataEvent<HitoBitFuturesStreamTradeUpdate>>? tradeHandler,
            ...
            Action<DataEvent<HitoBitGridUpdate>>? gridHandler,
            Action<DataEvent<HitoBitConditionOrderTriggerRejectUpdate>>? condOrderHandler) : base(logger, false)
        {
        ...
        public override CallResult DoHandleMessage(SocketConnection connection, DataEvent<object> message)
        {
            ...
            else if (message.Data is HitoBitCombinedStream<HitoBitConditionOrderTriggerRejectUpdate> condUpdate)
            {
                _condOrderHandler?.Invoke(message.As(condUpdate.Data, condUpdate.Stream, null, SocketUpdateType.Update));
            }
            else if (message.Data is HitoBitCombinedStream<HitoBitFuturesStreamTradeUpdate> tradeUpdate)
            {
